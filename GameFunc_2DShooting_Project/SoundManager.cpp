...
BOOL SoundManager::AddSoundBuffer(const string& key, LPDIRECTSOUNDBUFFER buffer)
{
...
void SoundManager::AddSound(const string & key, const string & wavPath, int flag)
{
	AddSoundBuffer(key, LoadWav(wavPath, flag));
}

void SoundManager::DeleteSound(const string& key)
...
LPDIRECTSOUNDBUFFER SoundManager::LoadWav(const string& path, int Flag)
{
...
void SoundManager::Stop(const string& key)
{
...
void SoundManager::Play(const string& key, BOOL Loop)
{
	if (IsBufIndex(key))
	{
		if (IsPlaying(key))
...
void SoundManager::DuplicatePlay(const string & key)
{
	if (IsBufIndex(key))
	{
		if (IsPlaying(key))
...
void SoundManager::DuplicateDistancePlay(const string & key, Vector3 position)
{
	if (IsBufIndex(key))
	{
		if (IsPlaying(key))
		...
		Vector3 listenPos = CAMERA.GetScroll();
...
BOOL SoundManager::IsBufIndex(const string& key)
{
	auto find = m_mapSoundBuffers.find(key);
...
void SoundManager::SetPan(const string& key, long pan)
{
...
void SoundManager::SetVol(const string& key, int volPercent)
{
...
int  SoundManager::GetVol(const string & key)
{
	if (IsBufIndex(key))
	{
		long pOut;
...
void SoundManager::AddVol(const string & key, int volPercent)
{
	if (IsBufIndex(key))
	{
		long pOut;
